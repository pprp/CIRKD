...
def parse_args():
    ...
    parser.add_argument('--contrast-kd-temperature',
                        type=float,
                        default=1.0,
                        help='similarity distribution KD temperature')
    parser.add_argument('--contrast-temperature',
                        type=float,
                        default=0.1,
                        help='similarity distribution temperature')

    parser.add_argument('--lambda-kd',
                        type=float,
                        default=1.,
                        help='lambda_kd')
    parser.add_argument('--lambda-minibatch-pixel',
                        type=float,
                        default=1.,
                        help='lambda mini-batch-based pixel')
    parser.add_argument('--lambda-memory-pixel',
                        type=float,
                        default=0.1,
                        help='lambda memory-based pixel')
    parser.add_argument('--lambda-memory-region',
                        type=float,
                        default=0.1,
                        help='lambda memory-based region')
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'citys':
            ...
            train_dataset = VOCDataTrainSet(args.data,
                                            './dataset/list/voc/train_aug.txt',
                                            max_iters=args.max_iterations *
                                            args.batch_size,
                                            crop_size=args.crop_size,
                                            scale=True,
                                            mirror=True)
            val_dataset = VOCDataValSet(args.data,
                                        './dataset/list/voc/val.txt')
        elif args.dataset == 'camvid':
            train_dataset = CamvidTrainSet(
                args.data,
                './dataset/list/CamVid/camvid_train_list.txt',
                max_iters=args.max_iterations * args.batch_size,
        ...
        self.criterion_kd = CriterionAutoKD()
                
        self.criterion_minibatch = CriterionMiniBatchCrossImagePair(
            temperature=args.contrast_temperature).to(self.device)
        
        self.criterion_memory_contrast = StudentSegContrast(
            num_classes=train_dataset.num_class,
            pixel_memory_size=args.pixel_memory_size,
            region_memory_size=args.region_memory_size,
            region_contrast_size=args.region_contrast_size //
            train_dataset.num_class + 1,
            pixel_contrast_size=args.pixel_contrast_size //
            train_dataset.num_class + 1,
        ...
        if args.distributed:
            ...
            self.criterion_memory_contrast = nn.parallel.DistributedDataParallel(
                self.criterion_memory_contrast,
                device_ids=[args.local_rank],
                output_device=args.local_rank)
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            minibatch_pixel_contrast_loss_reduced = self.reduce_mean_tensor(
                minibatch_pixel_contrast_loss)
            memory_pixel_contrast_loss_reduced = self.reduce_mean_tensor(
                memory_pixel_contrast_loss)
            memory_region_contrast_loss_reduced = self.reduce_mean_tensor(

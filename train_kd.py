...
def parse_args():
    ...
    parser.add_argument('--student-model',
                        ...
                        default='deeplabv3',
                        help='model name')
    parser.add_argument('--student-backbone',
                        type=str,
                        default='resnet18',
                        help='backbone name')
    parser.add_argument('--teacher-backbone',
                        type=str,
                        default='resnet101',
                        help='backbone name')
    ...
    parser.add_argument('--ignore-label',
                        type=int,
                        default=-1,
                        metavar='N',
                        help='ignore label')
    ...
    parser.add_argument('--kd-temperature',
                        type=float,
                        default=1.0,
                        help='logits KD temperature')
    parser.add_argument('--lambda-kd',
                        type=float,
                        default=0.,
                        help='lambda_kd')
    parser.add_argument('--lambda-adv',
                        type=float,
                        default=0.,
                        help='lambda adversarial loss')
    parser.add_argument('--lambda-d',
                        type=float,
                        default=0.,
                        help='lambda discriminator loss')
    parser.add_argument('--lambda-skd',
                        type=float,
                        default=0.,
                        help='lambda skd')
    parser.add_argument('--lambda-cwd-fea',
                        type=float,
                        default=0.,
                        help='lambda cwd feature')
    parser.add_argument('--lambda-cwd-logit',
                        type=float,
                        default=0.,
                        help='lambda cwd logit')
    parser.add_argument('--lambda-ifv',
                        type=float,
                        default=0.,
                        help='lambda ifvd')
    parser.add_argument('--lambda-fitnet',
                        type=float,
                        default=0.,
                        help='lambda fitnet')
    parser.add_argument('--lambda-at',
                        type=float,
                        default=0.,
                        help='lambda attention transfer')
    parser.add_argument('--lambda-psd',
                        type=float,
                        default=0.,
                        help='lambda pixel similarity KD')
    parser.add_argument('--lambda-csd',
                        type=float,
                        default=0.,
    ...
    parser.add_argument('--save-per-iters',
                        type=int,
                        default=800,
                        help='per iters to save')
    parser.add_argument('--val-per-iters',
                        type=int,
                        default=800,
                        help='per iters to val')
    parser.add_argument('--teacher-pretrained-base',
                        type=str,
                        default='None',
                        help='pretrained backbone')
    parser.add_argument('--teacher-pretrained',
                        type=str,
                        default='None',
                        help='pretrained seg model')
    parser.add_argument('--student-pretrained-base',
                        type=str,
                        default='None',
                        help='pretrained backbone')
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.t_model = get_segmentation_model(
        ...
        self.s_model = get_segmentation_model(
        ...
        self.D_model = Discriminator(preprocess_GAN_mode=1,
        ...
        self.criterion_kd = CriterionKD(temperature=args.kd_temperature).to(
            self.device)
        ...
        self.criterion_ifv = CriterionIFV(train_dataset.num_class).to(
            self.device)
        self.criterion_cwd = CriterionCWD(s_channels,
                                          t_channels,
                                          norm_type='channel',
                                          divergence='kl',
                                          temperature=4.).to(self.device)
        self.criterion_fitnet = CriterionFitNet(s_channels,
                                                t_channels).to(self.device)
        ...
        self.D_optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, self.D_model.parameters()), 4e-4,
            [0.9, 0.99])
        ...
        if args.distributed:
            ...
            self.D_model = nn.parallel.DistributedDataParallel(
                self.D_model,
                device_ids=[args.local_rank],
                output_device=args.local_rank)
            self.criterion_cwd = nn.parallel.DistributedDataParallel(
                self.criterion_cwd,
                device_ids=[args.local_rank],
                output_device=args.local_rank)
            self.criterion_fitnet = nn.parallel.DistributedDataParallel(
                self.criterion_fitnet,
                device_ids=[args.local_rank],
                output_device=args.local_rank)
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            if self.args.aux:
                task_loss = self.criterion(
                    s_outputs[0],
                    targets) + 0.4 * self.criterion(s_outputs[1], targets)
            else:
            ...
            adv_G_loss = self.args.lambda_adv * self.criterion_adv_for_G(
                self.D_model(s_outputs[0]))
            ...
            if self.args.lambda_kd != 0.:
                kd_loss = self.args.lambda_kd * self.criterion_kd(
                    s_outputs[0], t_outputs[0])
            if self.args.lambda_skd != 0:
                skd_loss = self.args.lambda_skd * self.criterion_skd(
                    s_outputs[-1], t_outputs[-1])
            if self.args.lambda_cwd_fea != 0:
                cwd_fea_loss = self.args.lambda_cwd_fea * self.criterion_cwd(
                    s_outputs[-1], t_outputs[-1])
            if self.args.lambda_cwd_logit != 0:
                cwd_logit_loss = self.args.lambda_cwd_logit * self.criterion_cwd(
                    s_outputs[0], t_outputs[0])
            if self.args.lambda_ifv != 0:
                ifv_loss = self.args.lambda_ifv * self.criterion_ifv(
                    s_outputs[-1], t_outputs[-1], targets)
            if self.args.lambda_fitnet != 0:
                fitnet_loss = self.args.lambda_fitnet * self.criterion_fitnet(
                    s_outputs[-1], t_outputs[-1])
            if self.args.lambda_at != 0:
                at_loss = self.args.lambda_at * self.criterion_at(
                    s_outputs[-1], t_outputs[-1])
            if self.args.lambda_psd != 0. and self.args.lambda_csd != 0.:
                ...
                psd_loss, csd_loss = self.criterion_dsd(
                    feat_s_list, feat_t_list)
            ...
            lr = self.adjust_lr(base_lr=args.lr,
                                iter=iteration - 1,
            ...
            eta_seconds = ((time.time() - start_time) /
                           iteration) * (args.max_iterations - iteration)
    ...
    def validation(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            B, H, W = target.size()
            outputs[0] = F.interpolate(outputs[0], (H, W),
                                       mode='bilinear',
                                       align_corners=True)
        ...
        if self.num_gpus > 1:
            ...
            sum_total_label = torch.tensor(self.metric.total_label).cuda().to(
                args.local_rank)
            sum_total_inter = torch.tensor(self.metric.total_inter).cuda().to(
                args.local_rank)
            sum_total_union = torch.tensor(self.metric.total_union).cuda().to(
                args.local_rank)
            ...
            pixAcc = 1.0 * sum_total_correct / (2.220446049250313e-16 +
                                                sum_total_label)
            IoU = 1.0 * sum_total_inter / (2.220446049250313e-16 +
                                           sum_total_union)
            mIoU = IoU.mean().item()

            logger.info(
                'Overall validation pixAcc: {:.3f}, mIoU: {:.3f}'.format(
        ...
        if (args.distributed is not True) or (args.distributed
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = 'kd_{}_{}_{}.pth'.format(args.student_model,
                                        args.student_backbone, args.dataset)
...
if __name__ == '__main__':
    ...
    logger = setup_logger('semantic_segmentation',
                          ...
                          filename='{}_{}_{}_log.txt'.format(
                              args.student_model, args.teacher_backbone,
                              args.student_backbone, args.dataset))

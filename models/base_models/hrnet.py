...
class HighResolutionModule(nn.Module):
    def __init__(self,
        ...
        self.branches = self._make_branches(num_branches,
                                            blocks,
                                            num_blocks,
                                            num_channels,
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    out_channels = num_channels_cur_layer[
                        i] if j == i - num_branches_pre else in_channels
                    conv3x3s.append(
                        nn.Sequential(
    ...
    def forward(self, x):
        ...
        for i in range(len(self.downsamp_modules)):
            y = self.incre_modules[i + 1](
                y_list[i + 1]) + self.downsamp_modules[i](y)
...
if __name__ == '__main__':
    ...
    model = HighResolutionNet(blocks, num_channels, num_modules, num_branches,
                              num_blocks, fuse_method)
    output = model(img)

...
class DeepLabV3(nn.Module):
    def __init__(self,
        ...
        self.pretrained = get_mobilenet_v2(pretrained=pretrained_base,
...
class ProjectionHead(nn.Module):
    def __init__(self, dim_in, norm_layer, proj_dim=256):
        ...
        self.proj = nn.Sequential(nn.Conv2d(dim_in, dim_in, kernel_size=1),
                                  norm_layer(dim_in), nn.ReLU(True),
...
class _DeepLabHead(nn.Module):
    def __init__(self,
        ...
        self.aspp = _ASPP(in_channels, [12, 24, 36],
        ...
        self.block = nn.Sequential(
            ...
            nn.ReLU(True), nn.Dropout(0.1), nn.Conv2d(out_channels, nclass, 1))
...
class _ASPP(nn.Module):
    def __init__(self, in_channels, atrous_rates, norm_layer, norm_kwargs,
        ...
        self.b3 = _ASPPConv(in_channels, out_channels, rate3, norm_layer,
                            norm_kwargs)
        self.b4 = _AsppPooling(in_channels,
                               out_channels,
                               norm_layer=norm_layer,
                               norm_kwargs=norm_kwargs)
...
def get_deeplabv3_mobile(backbone='resnet50',
                         local_rank=None,
                         pretrained=None,
                         pretrained_base=True,
                         num_class=19,
                         **kwargs):
    model = DeepLabV3(num_class,
                      backbone=backbone,
                      local_rank=local_rank,

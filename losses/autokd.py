...
class HintLoss(nn.Module):
    ...
    def forward(self, f_s, f_t, opt):
        loss = self.crit(f_s, f_t)
        return f_loss(f_s, f_t, opt.use_att, opt.use_ms, opt.use_local,
                      opt.use_batch, opt.use_channel, opt.use_kl)
...
class CriterionAutoKD(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        for scale in self.ms:
            loss.append(channel_kl(trans_ms(x1, scale), trans_ms(x2, scale)))
        loss_tensor = torch.Tensor(loss).to(x1.device)

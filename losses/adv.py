...
__all__ = ['CriterionAdvForG', 'CriterionAdv']
...
class CriterionAdv(nn.Module):
    ...
    def forward(self, d_out_S, d_out_T):
        assert d_out_S[0].shape == d_out_T[
            0].shape, 'the output dim of D with teacher and student as input differ'
        '''teacher output'''
...
class CriterionAdditionalGP(nn.Module):
    ...
    def forward(self, d_in_S, d_in_T):
        ...
        interpolated = torch.tensor(alpha * real_images.data +

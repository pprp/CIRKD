...
from utils.distributed import (get_rank, make_batch_data_sampler,
...
def parse_args():
    ...
    parser.add_argument('--data-list',
                        type=str,
                        default='./dataset/list/cityscapes/val.lst',
                        help='dataset directory')
    ...
    parser.add_argument('--scales',
                        ...
                        type=float,
                        nargs='+',
                        help='multiple scales')
    parser.add_argument('--flip-eval',
                        action='store_true',
                        default=False,
...
class Evaluator(object):
    def __init__(self, args, num_gpus):
        ...
        if args.dataset == 'citys':
            self.val_dataset = CSValSet(args.data,
                                        './dataset/list/cityscapes/val.lst',
                                        crop_size=(1024, 2048))
        elif args.dataset == 'voc':
            self.val_dataset = VOCDataValSet(args.data,
                                             './dataset/list/voc/val.txt')
        elif args.dataset == 'camvid':
            self.val_dataset = CamvidValSet(
                args.data, './dataset/list/CamVid/camvid_test_list.txt')

        val_sampler = make_data_sampler(self.val_dataset, False,
                                        args.distributed)
        ...
        with torch.no_grad():
            logger.info('Params: %.2fM FLOPs: %.2fG' %
                        (cal_param_size(self.model) / 1e6,
                         cal_multi_adds(self.model, (1, 3, 1024, 2048)) / 1e9))
    ...
    def predict_whole(self, net, image, tile_size):
        interp = nn.Upsample(size=tile_size,
                             mode='bilinear',
                             align_corners=True)
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            full_probs = torch.zeros(
                (1, self.val_dataset.num_class, H_, W_)).cuda()
            scales = args.scales
            with torch.no_grad():
                for scale in scales:
                    scale = float(scale)
                    ...
                    scale_image = F.interpolate(image,
                                                ...
                                                mode='bilinear',
                                                align_corners=True)
                    scaled_probs = self.predict_whole(model, scale_image,
                                                      tile_size)

                    if args.flip_eval:
                        print('flip evaluation')
                        flip_scaled_probs = self.predict_whole(
                            model, torch.flip(scale_image, dims=[3]),
                            tile_size)
                        scaled_probs = 0.5 * (scaled_probs + torch.flip(
                            flip_scaled_probs, dims=[3]))
...
if __name__ == '__main__':
    ...
    if args.save_pred:
        if (args.distributed
                and args.local_rank == 0) or args.distributed is False:
    ...
    logger = setup_logger('semantic_segmentation',
                          args.save_dir,
                          get_rank(),
                          filename='{}_{}_{}_multiscale_val.txt'.format(
